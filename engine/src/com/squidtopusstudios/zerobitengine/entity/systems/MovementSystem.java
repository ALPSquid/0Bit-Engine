...
public class MovementSystem extends ZbeSystem {
    ...
    private DynamicEntity solidEntity;

    @Override
    public void processEntity(ZbeEntity entity, float deltaTime) {
        /*if (!entity.getMetaData().isBox2D) {
            DynamicEntity dynamicEntity = (DynamicEntity)entity; // Fun Fact: Doing this instead of downcasting the below use cases
            ...
            if (ComponentMappers.physics.get(entity).applyGravity) {
                dynamicEntity.increaseVelocity(0, -ZeroBit.getWorld().getGravity() * deltaTime);
            }
            moveBy(dynamicEntity, dynamicEntity.getVelocity().x, dynamicEntity.getVelocity().y, true);
        } else {
     ...
     */
    public void moveBy(DynamicEntity entity, float x, float y, boolean interpolate) {
        ...
        /*if (entity.collisionsEnabled()) {
            entity.getCollisionData().collidedRight = false;
            entity.getCollisionData().collidedLeft = false;
            entity.getCollisionData().collidedTop = false;
            entity.getCollisionData().collidedBottom = false;
            // Move x
            for (String solidType : entity.getSolidTypes()) {
                ...
                if (solidEntity != null) {
                    if ( x > 0) {
                        ...
                        entity.getCollisionData().collidedRight = true;
                    } else if (x < 0) {
                        // Moving left
                        x = (solidEntity.getX() + solidEntity.getWidth()) - entity.getX();
                        entity.getCollisionData().collidedLeft = true;
                    }
                    break;
                }
            }
            for (String solidType : entity.getSolidTypes()) {
                ...
                if (solidEntity != null) {
                    if (y > 0) {
                        ...
                        entity.getCollisionData().collidedTop = true;
                        ...
                        entity.getCollisionData().collidedBottom = true;
