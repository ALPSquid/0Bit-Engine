...
public abstract class Logger {
    ...
    private void log(String message, Throwable exception, LogType type) {
        if (Gdx.app.getLogLevel() != LOG_NONE) {
            ...
            write(tag, message, exception, type);
            if (nextLogger != null) nextLogger.write(tag, message, exception, type);
    ...
    private void getCallerDetails() {
        ...
        for (int i=4; i < stackTraceElements.length; i++) {
            if (!stackTraceElements[i].getClassName().contains("Logger")) {
                ...
                lineNumber = stackTraceElements[i+1].getLineNumber();
                System.out.println("Logger Stack Trace index: " + (i+1));
                break;
            }
        }
        callerDetails.clear();
